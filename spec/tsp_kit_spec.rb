...
describe TspKit do
  describe 'random number generator' do
    ...
    it 'generates numbers consistently when seeded' do
      random_sequences.each do |seed, expected|
        described_class.srand(seed)
        actual = expected.map { described_class.rand }
        expect(actual).to match(expected.map { |value| be_within(1e-6).of(value) })
      end
    end

    it 'can accept seeds which are arrays of integers' do
      array_seed_sequences.each do |seed, expected|
  ...
  describe '.randn' do
    ...
    it 'generates sequences from normal distribution' do
      normal_sequences.each do |seed, expected|
        described_class.srand(seed)
        actual = expected.map { described_class.randn }
        expect(actual).to match(expected.map { |value| be_within(1e-6).of(value) })
      end
    end
  end
  ...
  describe '.shuffle_narray' do
    ...
    it 'shuffles numbers consistently when seeded' do
      shuffle_sequences.each do |seed, expected|
        expect(shuffled_values(described_class, seed)).to eql(expected)
      end
    end
    ...
    it 'can shuffle a large array' do
      ...
      expect(narray[500..519].to_a).to eql [
        37_544, 132_336, 13_460, 78_671, 46_310, 44_317, 126_946, 113_401, 45_819, 56_437,
        69_486, 18_315, 20_744, 172_846, 54_965, 82_657, 70_511, 5599, 44_508, 27_858
      ]
    end
  end
end
